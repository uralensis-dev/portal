  ...
  private $group_type = "";
    ...
    public function type_wise_report() {
      ...
      foreach ($list as $record) {
        ...
        $row[] = '<a href="' . site_url() . 'uploads/reports_pdf/' . $record->uralensis_request_id ."_".date("Y").".pdf" . '"><img src="' . base_url('assets/img/adobe.png') . '" />&nbsp; Docs</a>';
        $data[] = $row;
        $flag_count++;
      }
      $output = array(
  ...
  public function saveHospitalData() {
    ...
    public function addBookingDataFromCsv() {
        ...
        if($_REQUEST) 
		{
            ...
            if (!empty($template_data)) 
			{
              if($template_data['temp_labId'] != 0) $labId = $template_data['temp_labId'];
              $hospital_id = $template_data['hospital_id'];
              $clinic_user_id = $template_data['temp_clinic_user'];
              $pathologist_id = $template_data['temp_pathologist'];
