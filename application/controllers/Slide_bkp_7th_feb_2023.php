...
class Slide extends CI_Controller
{
    ...
    public function AddSlides($fileName = '', $fileId = '')
    {
        ...
        /*if (!strpos(urldecode($fileName), '.mrxs')) { 
        ...
        if ($imageInfo && count($imageInfo) > 0) {
            $bInfo = explode(",", $imageInfo[2]);
            $labNumber = $imageInfo[0] . "-" . $imageInfo[1];
            ...
            if ($requestId == '') {
                ...
                $requestedSpecimenNo = (int) filter_var($bInfo[0], FILTER_SANITIZE_NUMBER_INT);
                if ($requestedSpecimenNo == 0) {
                    ...
                    if ($specimen_data == '') {
                        ...
                        $blocksInfo = $this->sl->getSpecimensBlocks($specimen_data, $bInfo[0], $bInfo[1]);
					    if (!$blocksInfo) 
						{
                            ...
                            $blockSlides['url'] = "https://pcislides.pathhub.live/slide/".$fileId."/";
                            $blockSlides['thumbnail'] = "https://pcislides.pathhub.live/slide/".$fileId."/fullthumbnail";
