...
class Document extends CI_Controller
{
    ...
    {   	     
		...
		$result = $this->Document_List_Model->fetch_document_list();
				
        $data['total_document'] = count($result);
		
		$result = $this->db->query("SELECT id  FROM risk_register")->num_rows();
        $data['total_risk'] = $result;
		$result_request = $this->Document_List_Model->received_request();
		$data['last5_document'] =$result_request;
		$user_id = $this->ion_auth->user()->row()->id;
		$decryptedDetails = $this->Userextramodel->getUserDecryptedDetailsByid($user_id);
		$data['user_info'] = $decryptedDetails;
