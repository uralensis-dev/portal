...
class GenerateBarcode extends CI_Controller {
				...
				if($lab_ids[$i] != '' || $digi_numbers[$i] != '' || $test_ids[$i] != ''){
					...
					if(empty($barcode_data))
					{
						...
						// if ($this->input->post('trackPage')) {
							$testsStrings = $this->input->post('test')[$i];
							if ($testsStrings  == 'null') {
								$testsStrings = "H&E";
							}
							$code = $digi_numbers[$i].",".$testsStrings;
							$specArray = explode(",",$this->input->post('specimen_id')[$i]);
							$si = 1;
	...
	public function download_barcode(){	
		...
		if(!$this->input->post('page')){
			foreach($this->input->post('ctest') as $key => $testData){
				...
				foreach($testList as $tkey => $test){
					...
					if($this->input->post('request_action') == 'downloadCassette'){
						...
						$books .= implode("-",array($hospitalInfo->channel_no,$this->input->post('lab_number')[$key], $tests[2],$this->input->post('ref_lab_number')[$key],$patientInfo))."\n";
			...
			if($this->input->post('request_action') == 'downloadCassette'){
				$books .= implode("-",array(($this->input->post('channel') == '0') ? "1":$this->input->post('channel'),$this->input->post('labNo'), $this->input->post('blockNo'),$this->input->post('ref_lab_number'),$this->input->post('patientName')))."\n";
			}else if($this->input->post('request_action') == 'downloadRequest'){
				//$books .= implode(",",array($this->input->post('lab_number')[$key], $this->input->post('patient_name')[$key]))."\n";
