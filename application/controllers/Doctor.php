...
Class Doctor extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->helper(array('url', 'activity_helper', 'dashboard_functions_helper', 'datasets_helper', 'ec_helper', 'globalfunctions_helper'));
...
//                Months Not Exists
	...
	public function doctor_record_detail_old($id, $view = '') {
		...
		if (isset($id) && !empty($id)) {
			...
			if (count($res) == 0 || $res[0]['user_id'] == 0) {
				if ($user_type == 'D') {
					redirect("doctor/doctor_record_list");
				}
				if ($user_type == 'LA' || $user_type == 'LS' || $user_type == 'DE' || $user_type == 'LC') {
					redirect("lab/lab_record_list");
				}
				if ($user_type == 'HA' || $user_type == 'C' || $user_type == 'HS') {
					redirect("institute/view_request_detailall");
				}
				exit;
			}
			...
			$specimen_data['specimen_query'] = $this->Doctor_model->doctor_record_detail_specimen($id, $main_id->hospital_group_id);

			$specimen_data['specimen_query'][0]->patientInfo = $this->Doctor_model->get_patient_details($main_id->p_id);
			$slide_data['slide_data'] = $this->Doctor_model->get_case_slides_data($id);
			...
			$data['request_query'][0]->group_type = $group_type;
			...
			$datasets_data['datasets'] = $this->Doctor_model->get_datasets_data();
			$record_history['record_history'] = $this->Doctor_model->get_record_history_data($id);
			$data['request_query'][0]->block_history = $this->Doctor_model->get_record_block_history_data($id);
			$mdt_assign_dates['mdt_assign_dates'] = $this->Doctor_model->get_db_assign_dates($id);
			$download_history['download_history'] = $this->Doctor_model->getRecordDownloadHistory($id, $doctor_id);
			$specimen_accepted_by['specimen_accepted_by'] = $this->Doctor_model->get_specimen_data('specimen_accepted_by');
			$specimen_assisted_by['specimen_assisted_by'] = $this->Doctor_model->get_specimen_data('specimen_assisted_by');
			...
			$specimen_labeled_by['specimen_labeled_by'] = $this->Doctor_model->get_specimen_data('specimen_labeled_by');
			$specimen_qcd_by['specimen_qcd_by'] = $this->Doctor_model->get_specimen_data('specimen_qcd_by');
			$specimen_data['specimen_blocks'] = $this->Doctor_model->get_specimen_block_detail($id);
			...
			$this->db->update('request', $change_status);
			$files_data["files"] = $this->Doctor_model->fetch_files_data($id);
			$specimen_data['specimen_query'][0]->files = $this->Doctor_model->fetch_files_data($id);
		}
		$breadcrumb['breadcrumbs'] = $this->mybreadcrumb->render();
		$cv_appr_data = array(
			'gmc_no' => $this->getUserMetaData($doctor_id, 'gmc_no'),
...
//        pre($data);
		...
		require_once 'application/views/doctor/block_history/block_history_function.php';
		if ($view == 'view') {
			$this->load->view('doctor/inc/header-new');
			$search_query = explode("_", $this->input->get('slide'));
			if ($search_query != null) {
	...
	public function doctor_record_detail_old_new($id, $view = '') {
...
//        pre($data);
		...
		require_once 'application/views/doctor/manage_documents_old.php';
		require_once 'application/views/doctor/related_posts_old.php';
		require_once 'application/views/doctor/get_specimens_old_new.php';
		require_once 'application/views/doctor/special_notes_old.php';
		require_once 'application/views/doctor/inc/functions_old.php';
		require_once 'application/views/doctor/datasets/datasets.php';
		require_once 'application/views/doctor/record_history/record_history-functions_old.php';
		if ($view == 'view') {
			...
			$this->load->view('doctor/inc/header-new');
			$this->load->view('doctor/record_detail_old_new', $data_and_files);
			$this->load->view('doctor/inc/footer-new', $footer_data);
		}
	}
	...
	public function doctor_record_detail_new($id, $view = '') {
		...
		if (isset($id) && !empty($id)) {
			...
			$patient_q['patient_query'] = $this->Doctor_model->get_patient_details($main_id->p_id);
			$whereArr = [
				'request_id' => $main_id->uralensis_request_id,
				//'created_by' => $this->ion_auth->user()->row()->id
			];
			$additional_work = $this->db->order_by("additional_id desc")->get_where('additional_work', $whereArr)->result_array();
			$data['request_query']['0']->additional_work = $additional_work;
			//print_r($data_patient_id); exit;
			$user_type = $this->Doctor_model->get_user_type($doctor_id);
			switch ($user_type) {
			case 'A':
	...
	public function generate_report($id, $action = '', $actionType='') {
		...
		if (isset($id) && !empty($id)) {
			...
			*/
			...
			$reqData = $this->db->get_where('request', ['uralensis_request_id' => $id])->row();
			$labReportId = (!empty($reqData->lab_report)) ? $reqData->lab_report : $reqData->lab_id;

			$template_id = PCI_REPORT;
			if ($req_data['lab_processing_id'] != 114) {
				$template_id = PCI_UEALENSIS;
			}
			$templateArr = $this->db->order_by('id', 'desc')->get_where('lab_templates', ['id' => $template_id])->row_array();

			if (!$templateArr) {
				$whereArr = ['created_by' => $this->ion_auth->user()->row()->id];
				$templateArr = $this->db->get_where('lab_templates', $whereArr)->row_array();
			}

			$copy_to_clinician = $this->db->select('CONCAT(AES_DECRYPT(u.first_name, "' . DATA_KEY . '")," ",AES_DECRYPT(u.last_name, "' . DATA_KEY . '")) as clinicianNames')
				->join('users u', 'u.id = rc.clinician_id', 'inner')
				->where('rc.request_id', $id)
				->get('request_clinicians rc')
				->result_array();
			$clinicianInfo = '';
			if (count($copy_to_clinician) > 0) {
				foreach ($copy_to_clinician as $ckey => $cli) {
					if ($clinicianInfo == '') {
						$clinicianInfo = $cli['clinicianNames'];
					} else {
						$clinicianInfo .= ", " . $cli['clinicianNames'];
					}
				}
			}
			$data1['copy_to'] = $clinicianInfo;
			...
			*/
			...
			$res = array();
			$res = $this->db->query("SELECT g.*,hi.*  FROM `groups` as g INNER JOIN hospital_information hi on hi.group_id = g.id LEFT JOIN request r on r.hospital_group_id = g.id WHERE r.`uralensis_request_id` = $id LIMIT 1")->row_array();
			$data1['hospitalInformation'] = $res;

			$result = array_merge($data1, $data2, $data3, $data4, $data5, $data6, $lab_release_date, $action_array);
	...
	public function update_client_report() {
		...
		if (isset($_POST)) {
			...
			$spec = array(
				...
				'specimen_microscopic_code' => ($this->input->post('specimen_microscopic_code')) ? implode(",", $this->input->post('specimen_microscopic_code')) : "",
				...
				'specimen_special_notes' => $this->input->post('flag_commentss'),
	...
	public function additional_work() {
		...
		if ($this->input->post('additional_id') && $this->input->post('additional_id') != '0') {
			$this->db->where('additional_id', $this->input->post('additional_id'));
			$this->db->update('additional_work', array('description' => $this->input->post('additional_description')));
		} else {
		...
		if ($this->input->post('additional_id') && $this->input->post('additional_id') != '0') {
			redirect('doctor/doctor_record_detail_new/' . $req_id, 'refresh');
		} else {
	...
	public function display_published_reports_ajax_processing() {
		...
		$list = $this->Doctor_model->display_published_record($url_year, $url_type, $flag_type, $sort_authorize, $urgency_type, $row_color_code, $viewType);
		...
		foreach ($list as $record) {
			...
			// Modal comment get here
			$user_id = $this->ion_auth->user()->row()->id;
			$whereArr = ['record_id' => $record->uralensis_request_id, 'user_id' => $user_id, 'module_id' => 1];
			$comment = $this->db->get_where("section_comments", $whereArr)->row()->description;
			if (!empty($comment)) {
				$class = "color: #70cafb;background: #cef4fe;padding: 5px;border-radius: 50%;";
			} else {
				$class = "";
			}

			$add_comments .= '<i class="lnr lnr-bubble" style="font-size:18px;font-weight:bold;' . $class . '"></i>';
			$add_comments .= '</a>';
			$add_comments .= '</div>';
			...
			$add_comments .= '<textarea name="flag_comment" id="flag_comment" class="form-control flag_comment">' . $comment . '</textarea>';
			$add_comments .= '</div>';
			$add_comments .= '<div class="form-group">';
			$add_comments .= '<hr>';
			...
			$row[] = uralensisGetUsername($record->dermatological_surgeon, 'fullname');

			// $row[] = $record->added_by;
			//$row[] = '<a href="" target="_blank"><img src="https://portal.pathhub.live/assets/img/pdf.png" title="Pdf View"></a>';
			...
			// $row[] = $record->added_by;
			if ($groupType != "L" AND $groupType != "H") {
		...
		$output = array(
			...
			"recordsFiltered" => $this->Doctor_model->record_count_filtered($url_year, $url_type, $flag_type, $sort_authorize, $urgency_type, $row_color_code, $viewType),
			"data" => $data,
		);
	...
	public function display_published_reports_ajax_processing_dr() {
		$url_year = '2022';
		$url_type = '';
		$viewType = 2;
		if (!empty($_POST['year']) && !empty($_POST['type'])) {
		...
		$list = $this->Doctor_model->display_published_record_dr($url_year, $url_type, $flag_type, $sort_authorize, $urgency_type, $row_color_code, $viewType);
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => intval($row_count),
			"recordsFiltered" => $this->Doctor_model->record_count_filtered_dr($url_year, $url_type, $flag_type, $sort_authorize, $urgency_type, $row_color_code, $viewType),
			"data" => $data,
		);
		echo json_encode($output);
	}
...
// error_reporting(E_ALL);
	...
	public function find_lab_number_records() {
		...
		if (!empty($_POST['lab_number'])) {
			$lab_number = $_POST['lab_number'];
			$find_lab = $this->db->query("SELECT * FROM request WHERE request.lab_number = '$lab_number' AND uralensis_request_id != '" . $_POST['request_id'] . "' LIMIT 1");
			$check_lab = $find_lab->result();
			if (!empty($check_lab)) {
				$json['type'] = 'error';
				$json['msg'] = 'Lab Number Already Exists.';
				...
				$json['type'] = 'success';
				$this->db->where('uralensis_request_id', $_POST['request_id']);
				$this->db->update('request', array('lab_number' => $lab_number));
				$json['msg'] = 'The lab number has been updated successfully!!!';
				echo json_encode($json);
				die;
			}
		}
	...
	public function updateRefLabNumber() {
		if (!$this->ion_auth->logged_in()) {
			redirect('auth/login', 'refresh');
		}
		$json = array();
		$json['type'] = 'error';
		// if (!empty($_POST['ref_lab_number'])) {
		$ref_lab_number = $_POST['ref_lab_number'];
		$find_lab = $this->db->query("SELECT * FROM request WHERE uralensis_request_id = '" . $_POST['request_id'] . "' LIMIT 1");
		$check_lab = $find_lab->result();
		if (!empty($check_lab)) {
			$json['type'] = 'success';
			$this->db->where('uralensis_request_id', $_POST['request_id']);
			$this->db->update('request', array('ref_lab_number' => $ref_lab_number));
...
//            $this->db->update("opinion_comment_likes",$dataArray);
	...
	public function set_input_change_color() {
		...
		if (isset($_POST['copy_to'])) {
			if (count($_POST['copy_to']) > 0) {
				foreach ($_POST['copy_to'] as $ckey => $clinicData) {
					$dermatological_surgeon = $clinicData;
					$res = $this->db->query("SELECT * FROM `request_clinicians` WHERE `request_clinicians`.`request_id` = '" . $_POST["record_id"] . "' AND `request_clinicians`.`clinician_id` = $dermatological_surgeon")->result_array();
					if (count($res) == 0) {
						//default Clinician of the request
						$requestClinicianData = array(
							'request_id' => $_POST["record_id"],
							'clinician_id' => $dermatological_surgeon,
				...
				$this->db->where('request_id', $_POST["record_id"])->where_not_in('clinician_id', $_POST['copy_to'])->delete('request_clinicians');
			}
		} else {
			$this->db->where('request_id', $_POST["record_id"])->where_not_in('clinician_id', $_POST['copy_to'])->delete('request_clinicians');
		}
	...
	public function search_lab_number_mask() {
		...
		if (isset($_POST) && !empty($_POST['lab_id'])) {
			...
			if ($this->input->post('inputName') == 'lab_report') {
				$this->db->where('uralensis_request_id', $this->input->post('request_id'))->update('request', array('lab_processing_id' => $this->input->post('lab_id')));
			}
			$lab_mask = '';
...
//        redirect('/doctor/doctor_record_detail/' . $specimen_request_id);
		redirect('/doctor/doctor_record_detail_old/' . $specimen_request_id . "?spe=1", 'refresh');
	}
	...
	public function search_record_detail_suggestion() {
		...
		if (isset($_REQUEST['query'])) {
			$search_query = $_REQUEST['query'];
			if ($search_query == trim($search_query) && strpos($search_query, ' ')) {
				$searchString = explode(" ", $search_query);
				if (count($searchString) > 0) {
					if (substr_count($searchString[0], '-') > 1) {
						$labnumber = explode("-", $searchString[0]);
						$search_query = $labnumber[0] . "-" . $labnumber[1];
					} else {
						$search_query = $searchString[0];
					}
				}
			} else if (substr_count($search_query, '-') > 1) {
				$labnumber = explode("-", $search_query);
				$search_query = $labnumber[0] . "-" . $labnumber[1];
			}
			...
			foreach ($query->result() as $row) {
				...
				$array[$row->uralensis_request_id]['serial_number'] = $row->lab_number;
	...
	public function showSnomedCodes() {
		...
		$doctor_id = $this->ion_auth->user()->row()->id;
		$data['user_type'] = $this->Doctor_model->get_user_type($doctor_id);
		$this->load->view('doctor/inc/header-new');
		$this->load->view('doctor/snomed_codes', $data);
		$this->load->view('doctor/inc/footer-new');
	}
...
//                 Send email and attach uploaded images to email
	...
	public function addSpecimenBlock() {
		//$prefixes_data = getRecordLaboratoryPrefixes($this->input->post('brequestid'), $this->input->post('blabid'));
		if (!$this->ion_auth->logged_in()) {
			redirect('auth/login', 'refresh');
		}
		...
		if ($_POST['specimen_id']) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				for ($i = 0; $i < $this->input->post('count_blocks'); $i++) {
					$blockName = $this->input->post('block_no_of_blocks')[0];
					if ($i != 0) {
						$prefixes_data = getRecordLaboratoryPrefixes($this->input->post('brequestid'), $this->input->post('blabid'));
						$blockName = $prefixes_data[$specimen_id]['specimen_prefix'] . $prefixes_data[$specimen_id]['specimen_block_prefix'];
					}
					$defaultTestName = 'H&E';
					$insData[0] = [
						'specimen_id' => $specimen_id,
					...
					// Block history - add New Block
					$timestamp = strtotime('now');
					$record_data = array(
						'rec_history_user_id' => $this->ion_auth->user()->row()->id,
						'rec_history_record_id' => $this->input->post('request_id'),
						'rec_history_data' => $blockName,
						'rec_history_status' => 'add_block',
						'timestamp' => $timestamp,
					);
...
//                $insData['specimen_id'] = $specimen_id;
	...
	public function editSpecimenBlock() {
		...
		if ($_POST['sid'] && count($this->input->post('test_ids')) > 0) {
			...
			if (count($insData) > 0) {
				$res = $this->Doctor_model->add_specimen_block($insData);
				$record_data['rec_history_status'] = 'add_test';
				$record_data['rec_history_data'] = $insData[0]['block_no'] . " - " . $insData[0]['name'];
			} else {
				$tname = "'" . implode("', '", $testName) . "'";
				$sql = "SELECT block_no, name FROM specimen_blocks WHERE specimen_id = $specimen_id AND specimen_no = '" . $specimenNo . "' AND block_no = '" . $blockName . "' AND name  NOT IN (" . $tname . ")";
				$resArr = $this->db->query($sql)->row_array();
				$res = $this->Doctor_model->delete_specimen_block($specimen_id, $specimenNo, $blockName, $tname);
				$record_data['rec_history_data'] = $resArr['block_no'] . " - " . $resArr['name'];
				$record_data['rec_history_status'] = 'delete_test';
			}
...
//            echo "<Pre>";
	...
	public function LoadFurtherWorkData() {
		...
		$patientName = explode(" ", $this->input->post('patientName'));
		$patientInfo = '';
...
//        $datasets_cases['request_slides_id'] = $this->Doctor_model->doctor_record_list_with_slide($doctor_id);
	...
	public function delete_multiple_test($record_id, $rec = 0) {
		...
		//Getting block detail before delete
		$sql = "SELECT block_no from specimen_blocks WHERE id IN($test_ids) LIMIT 1";
		$resArr = $this->db->query($sql)->row_array();
		$this->db->query("DELETE FROM specimen_blocks WHERE id IN($test_ids)");
		$this->session->set_flashdata('success', TRUE);
		$this->session->set_flashdata('message', 'Test Deleted Successfully.');

		// Block history - Delete Block
		if ($resArr) {
			$timestamp = strtotime('now');
			$record_data = array(
				'rec_history_user_id' => $this->ion_auth->user()->row()->id,
				'rec_history_record_id' => $rec,
				'rec_history_data' => $resArr['block_no'],
				'rec_history_status' => 'delete_block',
				'timestamp' => $timestamp,
			);
			$this->db->insert('uralensis_block_history', $record_data);
		}
		...
		if ($_SERVER['REQUEST_METHOD'] == "POST") {
			...
			if ($patientArr) {
				...
				$reqData = array(
					...
					'dob' => $patientArr->dob,
					'age' => $age,
					'nhs_number' => $patientArr->nhs_number,
	...
	public function deleteAdditionalWork() {
		if (!$this->ion_auth->logged_in()) {
			redirect('auth/login', 'refresh');
		}
		$json = array();
		if (isset($_POST)) {
			$trash_id = $this->input->post('addtional_id');
			if ($this->db->query("DELETE FROM additional_work WHERE additional_id = $trash_id")) {
				$json['type'] = 'success';
				$json['msg'] = 'Addtional work has been deleted successfully!!!';
			} else {
				$json['type'] = 'fail';
