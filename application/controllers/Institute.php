...
class Institute extends CI_Controller {
	...
	public function index() {
		...
		$hospital_group_id = $this->ion_auth->get_users_main_groups()->row()->id;

		// $clinicInfo = base64_encode($this->Institute_model->getClinicDetails($user_id)['institute_id']);
		...
		$published['HAusers'] = $this->Huser_model->get_hospital_users('HA', 'count');
		$published['CSusers'] = $this->Huser_model->get_hospital_users('C', 'count');
		$published['Rusers'] = $this->Huser_model->get_hospital_users('R', 'count');
		$published['HSusers'] = $this->Huser_model->get_hospital_users('HS', 'count');
		...
		$decryptedDetails['decryptedDetails']->clinicInfo = $clinicInfo;
	 ...
	 * @return void
	 */
	public function reports($type = 'all', $viewType = '') {
		if (!$this->ion_auth->logged_in()) {
			redirect('auth/login', 'refresh');
		}
	...
	public function type_wise_report() {
		...
		foreach ($list as $record) {
			...
			if ($record->specimen_publish_status == 1) {
				$publish_status_download = '<a  href="' . site_url('Institute/download_pdf/' . $record->uralensis_request_id) . '"><img src="' . base_url('assets/img/download-1.png') . '">Download</a>';
			}
			$f_initial = '';
			$l_initial = '';
			...
			$row[] = '<a href="' . site_url() . 'doctor/doctor_record_detail_old_new/' . $record->uralensis_request_id . '"><img src="' . base_url('assets/img/detail.png') . '"></a>';
			$row[] = "Published";
			// $row[] = $publish_status;
			// $row[] = $publish_status_download;
			$pdfHtml = '<a title="View Report" href="' . site_url() . 'uploads/reports_pdf/' . $record->uralensis_request_id . "_" . date("Y") . ".pdf" . '" download><img src="' . base_url('assets/img/adobe.png') . '" /></a> &nbsp; ';
	...
	private function getTemplateDetails($user_id, $templatename) {
		...
		if (!empty($track_templates)) {
			...
			$templateArray = array(
				...
				'request_name' => $track_templates[0]->request_name,
				'category_name' => $track_templates[0]->category_name,
				'specimen_id' => $track_templates[0]->specimen_id_val,
				'tissue_type_id' => $track_templates[0]->tissue_type_id,
				'temp_labId' => $track_templates[0]->temp_labId,
			);
		}
...
//    public function bookingin()
	...
	public function bookingin() {
		...
		*/
		...
		$result_data["lab_info"] = $this->Laboratory_model->get_alllab_information(0);
	...
	public function getDepartmentSpecialityData() {
		...
		if ($this->input->post('action') == 'get_specimen_and_tissue_type') {
			$department_id = $this->input->post('department_id');
			$resArr = [
				'specimenArr' => $this->db->select('specimen_type_id as id, specimen_type as name')->get_where('department_settings_labs', ['department_id' => $department_id, 'specimen_type !=' => NULL])->result_array(),
				'tissueTypeArr' => $this->db->get_where('tissue_type', ['department_id' => $department_id])->result_array(),
			];
		} elseif ($this->input->post('action') == 'get_speciality') {
			$whereArr = [
				'specialty !=' => '',
		...
		} elseif ($this->input->post('action') == 'update_specimen') {
			$resArr = updateRecord('uralensis_record_track_template', ['specimen_id_val' => $this->input->post('specimen_id')], ['ura_rec_temp_id' => $this->input->post('template_id')]);
		} elseif ($this->input->post('action') == 'update_tissue_type') {
			$resArr = updateRecord('uralensis_record_track_template', ['tissue_type_id' => $this->input->post('tissue_type_id')], ['ura_rec_temp_id' => $this->input->post('template_id')]);
	...
	public function save_new_track_temp_data() {
		...
		if (!empty($_POST)) {
			...
			$tissue_type_id = $this->input->post('tissue_type_id');
			...
			if ($this->input->post('edit_mod') == "add" && empty($this->input->post('template_id'))) {
				...
				$temp_data = array(
					...
					'request_name' => !empty($request_name) ? $request_name : '',
				...
				$temp_data = array(
					...
					'department_id' => !empty($this->input->post('department_id')) ? $this->input->post('department_id') : '',
					'specimen_id_val' => !empty($this->input->post('specimen_id')) ? $this->input->post('specimen_id') : '',
					'tissue_type_id' => !empty($this->input->post('tissue_type_id')) ? $this->input->post('tissue_type_id') : '',
					'timestamp' => time(),
				);
	...
	public function add_patient_record() {
		...
		$lab_no = "PB" . date('y') . "-0" . rand(10000, 99999);
		...
		$blocks_data = array('specimen_id' => $specimen_id, 'specimen_no' => 1, 'block_no' => '1A', 'description' => 'HE');
		...
		$assign_request_args = array(
			...
			'publish_datetime' => date("Y-m-d h:i:s", strtotime("+1 month", strtotime(date("Y-m-d h:i:s")))),
	...
	public function batch_record() {
		...
		$blocks_data = array('specimen_id' => $specimen_id, 'specimen_no' => 1, 'block_no' => '1A', 'description' => 'HE', 'name' => 'H&E');
		$this->db->insert('specimen_blocks', $blocks_data);
		...
		$json['type'] = 'success';
		$json['msg'] = 'Record created successfully.';
		$json['redirect_url'] = base_url('doctor/doctor_record_detail_old/') . $record_last_id;
		echo json_encode($json);
		die;
	}
...
//                        echo 1; exit;
			...
			if ($this->form_validation->run() === TRUE) {
				...
				$this->Institute_model->save_hospital_data_new($main_group_info, $hospital_info);
				...
				$this->Institute_model->save_hospital_financedata_new($hospital_finance_info);
...
//            echo "<pre>";print_r($data['country']);exit;
			...
			if ($this->form_validation->run() === TRUE) {
				...
				if ($new_group_id) {
					...
					if (!empty($_POST['dac_checkbox'])) {
						...
						$profile_picture = DEFAULT_PROFILE_PIC;
						// Upload profile picture if exists
						if ($_FILES['dac_profile_pic']["name"]) {
							//when user submit basic profile info with profile image
							$config['upload_path'] = './uploads/';
							$config['allowed_types'] = 'gif|jpg|png';
							$config['max_size'] = '10000';
	...
	public function addBookingDataFromCsv() {
		...
		if ($_REQUEST) {
			...
			$speci_type = '';
			if (!empty($template_data)) {
				if ($template_data['temp_labId'] != 0) {
					$labId = $template_data['temp_labId'];
				}
			...
			if ($_FILES["UploadCSV"]["size"] > 0) {
				...
				/*if (!empty($f_initial) && !empty($l_initial)) {
					                    $last_d = str_pad($j, 4, "0", STR_PAD_LEFT);
					                    $batch_no = $f_initial . $l_initial . '.' . date('y') . '.' . $last_d;
										$lab_no="PB22-".$last_d."-".$f_initial.$l_initial;
					                } else {
					                    $last_d = str_pad($j, 4, "0", STR_PAD_LEFT);
					                    $batch_no = "PCI" . '.' . date('y') . '.' . $last_d;
										$lab_no="PB22-".$last_d."-".$f_initial.$l_initial;
				*/

				$postfix_serial_no = "-" . $f_initial . $l_initial;
				$prefix_lab_number = 'PCI-' . date('y');

				while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
					$flagged = 0;
					if ($i == 0) {
						if ($column[0] != 'Lab ID' && $column[1] != 'Patients' && $column[2] != 'DOB' && $column[3] != 'Specimen Letter' && $column[4] != 'Speciman Type' && $column[5] != 'Number of Sliders' && $column[6] != 'Request Date' && $column[7] != 'Status' && $column[8] != 'Date Taken') {
							$flagged = 1;
						}
					} else {
						$flagged = 1;
					...
					if ($flagged >= 1) {
						...
						if (isset($column[6])) {
							$request['report_urgency'] = $column[6];
						}
						$request['date_taken'] = (isset($column[7])) ? date('Y-m-d', strtotime($column[7])) : date('Y-m-d');

						if (isset($column[8])) {
							$request['request_datetime'] = date('Y-m-d H:i:s', strtotime($column[8]));
						}

						// $lab_no = serial_number('request', 'uralensis_request_id', $prefix_lab_number);
						// $lab_no="PB22-".rand(10000,99999);
						$lab_no = "PB" . date('y') . "-0" . rand(10000, 99999);

						$request['lab_id'] = $labId;
						$request['lab_name'] = "PCI";
						$request['emis_number'] = $courier_no;
						$request['template_id'] = $templateID;
						$request['record_batch_id'] = $batch_no;
						$request['hospital_group_id'] = $hospital_id;
						$request['lab_number'] = $lab_no;
						...
						if ($column[1] != '') {
							$key = serial_number('request', 'uralensis_request_id', 'PCI') . $postfix_serial_no;
							/*$get_serial_number = $this->db->query("SELECT * FROM request ORDER BY uralensis_request_id DESC LIMIT 1")->row_array();
								                            if ($get_serial_number == '') {
								                                $req_id_before_insert = 1;
								                            } else {
								                            ...
								                            if ($serial_query->num_rows() > 0) {
								                                ...
								                                if ($keyParts[1] == date('y')) {
								                                    $Klast_d = str_pad(($keyParts[2] + 1), 4, "0", STR_PAD_LEFT);
								                                    $key = 'PB22' . "." . $keyParts[1] . "." . ($Klast_d);
								                                } else {
								                                    $key = 'PB22' . "." . date("y") . ".00001";
								                                }
								                            } else if ($serial_query->num_rows() < 0) {
						...
						for ($i = 1; $i <= $specimen_no; $i++) {
							...
							if ($inserted_specimen_id > 0) {
								$this->db->insert('specimen_blocks', [
									...
									'specimen_no' => 1,
									'block_no' => (isset($column[3]) && !empty($column[3])) ? $column[3] : '',
									'description' => 'H&E',
	...
	public function get_specimen_by_department() {
		if ($_SERVER['REQUEST_METHOD'] == "POST") {
			$department_id = $this->input->post('department_id');
			$res = [
				'specimenArr' => $this->db->select('specimen_type_id as id, specimen_type as name')->get_where('department_settings_labs', ['department_id' => $department_id, 'specimen_type !=' => NULL])->result_array(),
			...
			echo json_encode(['status' => "success", 'data' => $res]);exit;
		}
	}

	public function MarkedAsViewed() {
		if ($this->input->post('changeId') && $this->input->post('changeId') != '') {
