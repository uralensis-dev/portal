...
public function delete_group($user_id) {
       if (!$this->ion_auth->logged_in() || (!$this->ion_auth->is_admin() && !($this->ion_auth->user()->row()->id == 385))) {
            redirect('auth', 'refresh');
        }       
...
//            exit;
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($current_email != $pre_email) {
                    ...
                    $this->db->where(array("useremail"=> $pre_email))->update('tbl_pins', $data);
                }
...
{
        $user_id = $this->ion_auth->user()->row()->id;
        $pin = $this->db->select('login_token')->where('id', 393)->get('users')->row_array();
        $login_token = $pin['login_token'];
        if(empty("$login_token")) 
		{
			
            echo json_encode("Token does not exist");
        }
        else {
            echo "$login_token";
        }
    ...
    public function delete_user_ajax() {
		...
		$user_id = $this->ion_auth->user()->row()->id;
        $pin = $this->db->select('login_token')->where('id', $user_id)->get('users')->row_array();
        $login_token = $pin['login_token'];
		
		if($passcode == "$login_token")
		{
    ...
    public function deleteSelectedUsers(){
        if($this->input->post('userIds')){
            ...
            $this->db->where_in('id', $userIds)->delete('users');
            // Permanent delete user
            // $this->db->where_in('id', $userIds)->update('users', ['deleted' => 1]);
            echo json_encode(['status'=>'success', 'message'=>'Selected records are delete successfully.']);exit;
        }else{
    ...
    public function check_session_time() 
	{
        ...
        if (time() > $_SESSION['expire'] or $status == "logout") {
            ...
            $_SESSION['expire'] = time() + $_SESSION['inactivity_min'] * 10;
            $response['timer'] = $_SESSION['expire'] - time();
        } else {
            if ($_SESSION['expire'] - time() <= 10) {
                $response['status'] = "error";
                $response['timer'] = $_SESSION['expire'] - time();
            } else {
                $response['status'] = "success";
    ...
    public function get_role_hospitals(){
        ...
        if($this->input->post()){
            ...
            if($groupType=="H" || $groupType=="L"){
                ...
                // $allGroups = $this->db->select("*")->get("groups")->result_array();
                $allGroups = $this->db->where(array("group_type"=>$group_type))->select("*")->get("groups")->result_array();
            }
...
//        echo "<pre>";print_r($data);exit;
    ...
    public function getRoleBasedGroup(){
        if (!$this->ion_auth->logged_in()) {
            redirect('auth/login', 'refresh');
        }
        $groupType = $this->input->post('group');
        $groupRole = $this->input->post('role');
        $html = '<option value="">Select '.$groupRole.'</option>';
        if($groupType){
            $query = $this->db->query("SELECT groups.*, hi.site_identifier, hi.identifier FROM `groups` LEFT JOIN `hospital_information` hi on hi.group_id=groups.id WHERE groups.group_type = '".$groupType."'");
            $result = $query->result();
            if($result){
                foreach ($result as $key => $clinic) {
                    $html .= '<option value="'.$clinic->id.'">'.$clinic->description.'</option>';
                }
            }
        }
        echo $html;
