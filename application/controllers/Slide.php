...
class Slide extends CI_Controller
{
    ...
    public function AddSlides($fileName = '', $fileId = '')
    {
        ...
        if ($imageInfo && count($imageInfo) > 0) {
            $labInfo = explode("-", $imageInfo[0]);
            $labNumber = $labInfo[0] . "-" . $labInfo[1];
            ...
            if ($requestId == '') {
                ...
                $requestedSpecimenNo = (int) filter_var($labInfo[2], FILTER_SANITIZE_NUMBER_INT);
                if ($requestedSpecimenNo == 0) {
                    ...
                    if ($specimen_data == '') {
                        ...
                        $blocksInfo = $this->sl->getSpecimensBlocks($specimen_data, $labInfo[2], $testName[0]);
