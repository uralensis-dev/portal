...
Class Doctor extends CI_Controller 
{
    ...
    public function doctor_record_list() 
    {
        ...
        $this->mybreadcrumb->add('<i class="lnr lnr-home"></i>', base_url('index.php/doctor'));
    ...
    public function further_work() 
    {
        ...
        if ($_POST['furtherwork_date'] == '') {
            ...
            $this->db->update('further_work', $fw_message);
                ...
                $this->email->to("i.h.chaudhry01@gmail.com");// change it to yours
    ...
    public function search_request() 
    {
        ...
        $gender = $this->input->post('gender');
        $data['query'] = $this->Doctor_model->get_search_request($first_name, $sur_name, $nhs_no, $dob, $gender);
        //echo last_query();exit;
        $this->load->view('doctor/inc/header-new');
    ...
    public function display_published_reports_ajax_processing() 
    {
        ...
        foreach ($list as $record) {
            ...
            } elseif (!empty($record->report_urgency) && $record->report_urgency === '2WW') {
    ...
    public function publish_bulk_reports_authrization() 
    {
        ...
        if (isset($_POST)) {
            ...
            //var_dump($user_pass);
         //   var_dump($auth_pass1."-".$auth_pass2."-".$auth_pass3."-".$auth_pass4);
    ...
    public function search_lab_number_mask() 
    {
        ...
        if (isset($_POST) && !empty($_POST['lab_id'])) {
            ...
            $find_lab_mask = $this->db->select('lab_mask')->where('id', $lab_id)->get('groups')->row_array();
            $lab_mask = '';
            if (!empty($find_lab_mask)) {
                $lab_mask = $find_lab_mask['lab_mask'];
                if (empty($lab_mask)) {
                    $json['type'] = 'error';
                    $json['msg'] = 'Sorry add the laboratory first and choose the default laboratory user in edit group section.';
                    echo json_encode($json);
                    die;
                } else {
                    $json['type'] = 'success';
     ...
     * Datasets Auto suggest
     */
    // public function _bk_dataset_autosuggest()
    // {
    //     if (!$this->ion_auth->logged_in()) {
    //         redirect('auth/login', 'refresh');
    //     }
    ...
    //         $sql = "SELECT * FROM uralensis_datasets WHERE uralensis_datasets.ura_datasets_name LIKE '%{$search_query}%'";
    //         $query = $this->db->query($sql)->result_array();

    //         $array = array();
    //         foreach ($dataset as $row) {
    //             $array[$row['ura_datasets_id']]['ura_datasets_id'] = $row['ura_datasets_id'];
    //             $array[$row['ura_datasets_id']]['ura_datasets_name'] = $row['ura_datasets_name'];
    //         }
    //         echo json_encode($array);
    //     }
    // }
     ...
     */
    // public function _bk_fetchDatasetCats()
    // {
    //     if (!$this->ion_auth->logged_in()) {
    //         redirect('auth/login', 'refresh');
    //     }
    ...
    //         //Fetch dataset name
    //         $dataset_query = $this->db->select('ura_datasets_name')->where('ura_datasets_id', $dataset_id)->get('uralensis_datasets')->row_array()['ura_datasets_name'];

    //         $query = $this->db->select('ura_datasets_cat_id, ura_dataset_cat_name')->where('ura_dataset_parent_id', $dataset_id)->get('uralensis_dataset_cats')->result_array();

    //         $dataset_name = '';
    //         if ($dataset_query) {
    //             $dataset_name = $dataset_query;
    //         }

    //         $cats_and_quest = array();
    //         if (!empty($query)) {
    //             foreach ($query as $key => $val) {
    //                 $cats_and_quest[$key]['cat_id'] = $val['ura_datasets_cat_id'];
    //                 $cats_and_quest[$key]['cat_name'] = $val['ura_dataset_cat_name'];
    //                 $cats_and_quest[$key]['questions'] = $this->getDatasetsQuestions($val['ura_datasets_cat_id'], $dataset_id, 'for_html');
    //             }
    //         }
     ...
     */
    // public function _bk_getDatasetsQuestions($cat_id = '', $dataset_id = '', $type = 'for_html')
    // {
    //     if (!$this->ion_auth->logged_in()) {
    //         redirect('auth/login', 'refresh');
    //     }
    ...
    //     if (!empty($cat_id)) {
    //         $ques_query = $this->db->select('ura_datasets_ques_id, is_required, ura_datasets_ques_type, ura_datasets_ques_title')
    //             ->where('ura_datasets_category_id', $cat_id)
    //             ->get('uralensis_datasets_questions')
    //             ->result_array();

    //         if (!empty($ques_query)) {
    //             foreach ($ques_query as $key => $val) {
                    
    //                 if (!empty($type) && $type === 'for_html') {
    //                     $ques_array[$key]['dataset_id'] = $dataset_id;
    //                     $ques_array[$key]['cat_id'] = $cat_id;
    //                     $ques_array[$key]['ques_id'] = $val['ura_datasets_ques_id'];
    //                     $ques_array[$key]['type'] = $val['ura_datasets_ques_type'];
    //                     $ques_array[$key]['title'] = $val['ura_datasets_ques_title'];
    //                     $ques_array[$key]['required'] = $val['is_required'];
    //                     $ques_array[$key]['answer_data'] = $this->getAnswerData($val['ura_datasets_ques_id'], 'for_html');
    //                     $ques_array[$key]['db_answer_list'] = $this->getAnswerData($val['ura_datasets_ques_id'], 'for_pdf');
    //                 } else {
    //                     $ques_array[$key]['ques_id'] = $val['ura_datasets_ques_id'];
    //                     $ques_array[$key]['type'] = $val['ura_datasets_ques_type'];
    //                     $ques_array[$key]['title'] = $val['ura_datasets_ques_title'];
    //                     $ques_array[$key]['answer_data'] = $this->getAnswerData($val['ura_datasets_ques_id'], 'for_pdf');
    //                 }
    //             }
    //         }
    ...
    public function printPDFDatasetData($dataset_id)
    {
        ...
        if (!empty($dataset_id)) {
            ...
            // $query = $this->db->select('ura_datasets_cat_id, ura_dataset_cat_name')
            //     ->where('ura_dataset_parent_id', $dataset_id)
            //     ->get('uralensis_dataset_cats')
            //     ->result_array();
