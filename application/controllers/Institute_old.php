    ...
    public function add_institute() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $request = array(
                ...
                'date_received_bylab' => $this->input->post('date_received_bylab'),
                'data_processed_bylab' => $this->input->post('data_processed_bylab'),
                'date_sent_touralensis' => $this->input->post('date_sent_touralensis'),
            ...
            /*Add Hospital Group ID into Request Table*/
            
            $hospital_group_id = array('hospital_group_id' => $this->ion_auth->group()->row()->id);
            $this->db->where('uralensis_request_id', $req_id);
            $this->db->update('request', $hospital_group_id);
           
            /*Add Hospital Group ID INTO users_request Table*/
            ...
            /* Initially Insert the Record ID in files Table */

//            $hospital_files_record_id = array('record_id' => $req_id);
//            $this->db->insert('files', $hospital_files_record_id);
...
//            $this->Doctor_model->additional_work_add($additional_work_add);
    ...
    public function viewed_reports() {
        ...
        $data["query"] = $this->Institute_model->institute_record_viewed();
...
//      $config['max_height'] = '768';
        ...
        if (!$this->upload->do_upload()) {
            ...
            $this->Institute_model->update_file(
                    $data['file_name'], $data['raw_name'], $data['full_path'], $data['file_ext'], $data['is_image'], $hospital_id, $user
            );
