...
class Auth extends CI_Controller {
    ...
    public function index() {
        ...
        $hospitalUserGroupArray = array("H", "C", "R", "CS", "NA", "HA", "AC","D");
...
//                echo '<pre>'.$value->user_id; print_r($user_hosp);
        ...
        } else if (!empty($groups) && $groups[0]->group_type === 'H' || $groups[0]->group_type === 'HA' ||(isset($get_group_type) && !empty($get_group_type) && is_array($get_group_type) && count($get_group_type) > 0 && $get_group_type[0]->group_type)) {
            track_user_activity();
...
public function delete_group($user_id) {
		...
		header('Content-Type: application/json');
        echo json_encode(array('status' => 'success'));
		redirect('institute/Hview', 'refresh');		
    }
    ...
    public function create_user() 
	{
        ...
        if ($this->form_validation->run() === TRUE) 
		{
            ...
            if ($user_ids) 
			{
                ...
                $this->changePasswordEmail($user_ids);
                $this->session->set_flashdata('message', $this->ion_auth->messages());
                redirect("auth", 'refresh');
                return;
            }
...
//        );
        ...
        $userDetails = $this->Institute_model->getUserDecryptedDetailsByid($userId);
        $message = $this->load->view("auth/email/password_change", array('userDetails' => $userDetails), TRUE);
...
//        $cid = $this->email->attachment_cid($logo);
        ...
        $this->email->to($email);
//        $this->email->set_header('Content-Type', 'text/html');
        $this->email->subject("Pathology Healthcare Change Password");
        $this->email->message($message);
        $this->email->send();
//        if(!$this->email->send()){
//            print_r($this->email->print_debugger());
...
//        echo "<pre>";print_r($this->data['usersLeaveBalance']);exit;
                ...
                if ($this->ion_auth->verify_password($prepass, $user->password) or ( $this->ion_auth->is_admin())) {
                    ...
                    if (!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 8) {
                        ...
                        if ($checkPass) {
                            ...
                            if($is_change==1){
                                $this->db->where(array("id"=>$user->id))->update("users",array("is_reset"=>1));
                            }
    ...
    public function password_change($getEmail=FALSE) {
        if (!$this->ion_auth->logged_in()) {
            redirect('auth/login', 'refresh');
        }
        $includes['styles'] = array('password/css/jquery.passwordRequirements.css');
        $includes['javascripts'] = array('password/js/jquery.passwordRequirements.min.js', 'password/js/custom.js');
        if($getEmail){
            $decryptData = encrypt_data($getEmail,"d");
            if($decryptData!=""){
                $explodeData = explode("____",$decryptData);
                $this->data['email'] = $explodeData[1];
                $this->data['user_id'] = $explodeData[0];
                $this->data['is_change'] = 1;

                $getData = $this->db->select("is_reset")->where(array("id"=>$explodeData[0]))->get("users")->row();
                if($getData->is_reset==1){
                    redirect("/");
                }
    ...
    public function get_role_hospitals(){
        ...
        if($this->input->post()){
            ...
            if($groupType=="D"){
                $allGroups = $this->db->where("group_type IN ('L','H')")->select("*")->get("groups")->result_array();
            } else {
