...
class Squamous_cell_dataset extends CI_Controller {
    ...
    public function dashboard() {
        ...
        if ($this->uri->segment(4) != '') {
            ...
            $data['get_record'] = $data['dataset_ssc'] = get_ssc_dataset_record($this->uri->segment(4), $this->uri->segment(13));
            $data['dataset_ssc_specimen'] = get_ssc_dataset_specimen($this->uri->segment(4));
            if (!empty($data['get_record'])) {
                $this->load->view('templates/header-new');
                $this->load->view('_dataset/squamous_cell_dataset/dashboard_edit', $data);
                $this->load->view('templates/footer-new');
            } else {
                $this->load->view('templates/header-new');
                $this->load->view('_dataset/squamous_cell_dataset/dashboard', $data);
                $this->load->view('templates/footer-new');
            }
        }
    }
...
//        _print_r($_POST); die();
        $ins_array = array(
            ...
            'ssc_data' => json_encode($this->input->post()),
            'ssc_response_html' => $this->input->post('ssc_respons_html'),
