...
//                echo $lab_id; exit; 
        ...
        if (isset($_FILES['upload_doc']) && count($_FILES['upload_doc']['name']) > 0) {
            ...
            foreach ($dataArr as $data){
                ...
                if (!empty($checklist_file_name)) {
                    ...
                    $reqArr = $this->db->get_where('request', ['pci_number'=>$data['raw_name']])->result_array();
                    if(count($reqArr) > 0 && !empty($reqArr[0]['pci_number'])){
                        $record_id = $reqArr[0]['uralensis_request_id'];
                        $file_tag = '';
                        $this->load->model('Doctor_model');
                        $this->Doctor_model->insert_file(
                            $data['file_name'], $data['raw_name'], $data['full_path'], $data['file_ext'], $data['is_image'], $user_name, $user_id, $record_id, $file_tag
                        );
    ...
    public function do_upload_lab_files($lab_file_name, $ref_key) {
        ...
        for($i = 0; $i < $filesCount; $i++){
            $_FILES['file']['name']     = $ref_key.'-'.$_FILES[$lab_file_name]['name'][$i];
            $_FILES['file']['type']     = $_FILES[$lab_file_name]['type'][$i];
            $_FILES['file']['tmp_name'] = $_FILES[$lab_file_name]['tmp_name'][$i];
            $_FILES['file']['error']    = $_FILES[$lab_file_name]['error'][$i];
            $_FILES['file']['size']     = $_FILES[$lab_file_name]['size'][$i];
        ...
        return ['error_upload' => $errorUploadType, 'success' => $fileData[0]];
