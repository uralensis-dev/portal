...
//        $count_data = $ci->Institute_model->further_view();
        ...
        $count_data = $ci->Doctor_model->display_further_work_requested();
...
if (!function_exists('getTotalNumberOfRecords')) {
    ...
    function getTotalNumberOfRecords()
    {
        ...
        $totalrecords = $ci->Admin_model->display_all_unpublishedrequested_Count();
...
if (!function_exists('getAssignedDoctorDetails')) {

    /**
     * Get  assigined doctor details
     *
     * @param string $request_id
     */
    function getAssignedDoctorDetails($request_id = '')
    {

        $ci = &get_instance();

        $sql = "SELECT u.profile_picture_path,AES_DECRYPT(u.phone, '" . DATA_KEY . "') AS phone,AES_DECRYPT(u.company, '" . DATA_KEY . "') AS company,AES_DECRYPT(u.last_name, '" . DATA_KEY . "') AS last_name,AES_DECRYPT(u.first_name, '" . DATA_KEY . "') AS first_name, AES_DECRYPT(u.email, '" . DATA_KEY . "') AS email, u.id,AES_DECRYPT(u.username, '" . DATA_KEY . "') AS username  FROM request_assignee ra
                LEFT JOIN request r ON r.uralensis_request_id = ra.request_id
                LEFT JOIN users u on u.id = ra.user_id
                WHERE r.uralensis_request_id=" . $request_id ." ORDER BY ra.req_assign_id DESC LIMIT 1";
...
if (!function_exists('get_profile_picture')) {
    ...
    function get_profile_picture($profile_picture_path, $first_name, $last_name)
    {
        ...
        elseif (empty($first_name) && empty($last_name)) {
            $profile_picture = UI_AVATAR . urlencode('N'.' '.'A');
        }
        else {
