...
class Chat_model extends CI_Model
{
	...
	$this->db->insert('chat_messages', $data);
		...
		function Insert_chat_message_group($data)
	{
	$this->db->insert('chat_messages_group', $data);
	}
	...
	function Fetch_chat_data_group($sender_id, $receiver_id)
	{
	$this->db->where('( group_id = "'.$receiver_id.'")');
	$this->db->order_by('chat_messages_id', 'ASC');
	return $this->db->get('chat_messages_group');
	}

	function Fetch_chat_data_group_last($sender_id, $receiver_id)
	{
	$this->db->where('(sender_id = "'.$sender_id.'" AND group_id = "'.$receiver_id.'")');
	$this->db->order_by('chat_messages_id', 'DESC');
	$this->db->limit(1);
	return $this->db->get('chat_messages_group');
	}
	...
	function Update_login_data($is_type,$receiver_id)
	{
	$data = array(
