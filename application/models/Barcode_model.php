...
Class Barcode_model extends CI_Model 
{
    ...
    public function get_bulk_barcode($ids){
        ...
        $this->db->select('br.*,spb.name as testName,spb.block_no as testBlockName, group_concat(DISTINCT(sp_block.name)) as testOld, group_concat(concat(sp_block.name,"_",sp.specimen_id,"_",sp_block.block_no) separator ",") as test,concat(rq.f_name," ",rq.sur_name) as patient_name, CONCAT(AES_DECRYPT(patho.first_name, "' . DATA_KEY . '")," " ,AES_DECRYPT(patho.last_name, "' . DATA_KEY . '")) AS pathologist');
        $this->db->from('barcode br');
        ...
        $this->db->join('specimen_blocks spb', 'spb.id = br.test_id', 'inner'); 
    ...
    public function get_request_bulk_barcode($ids){
        $subQuery = "(SELECT  req_assign_id,user_id,request_id,MAX(req_assign_id) Maxpath FROM  request_assignee GROUP BY req_assign_id) rq_as";
        $this->db->select('br.*,spb.name as testName,spb.block_no as testBlockName, group_concat(concat(sp_block.name,"_",sp.specimen_id,"_",sp_block.block_no) separator ",") as test, concat(rq.f_name," ",rq.sur_name) as patient_name, group_concat(DISTINCT(sp.specimen_id)) as sp_id, CONCAT(AES_DECRYPT(patho.first_name, "' . DATA_KEY . '")," " ,AES_DECRYPT(patho.last_name, "' . DATA_KEY . '")) AS pathologist');
        $this->db->from('barcode br');
        $this->db->join('request rq', 'rq.uralensis_request_id = br.request_id', 'inner'); 
        $this->db->join($subQuery, 'rq_as.request_id = rq.uralensis_request_id', 'inner');
        $this->db->join('specimen_blocks spb', 'spb.id = br.test_id', 'inner'); 
