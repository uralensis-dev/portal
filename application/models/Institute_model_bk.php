...
Class Institute_model extends CI_Model
{
    ...
    private function _get_datatables_published_record_query()
    {
        ...
        $this->db->where('req.specimen_publish_status', '1');
      ...
      AND reqv.user_viewed_id = $user_id
    )";
    ...
    {
		$query = $this->db->query("SELECT * FROM tbl_courier WHERE created_by = $user_id");
        return $query->result();
	}
    ...
    public function get_all_track_record_templates_bynameLike($user_id)
    {
        ...
        if ($is_admin) {
          ...
          WHERE temp_status='1'");
            ...
            $query = $this->db->query("SELECT ura_rec_temp_id,temp_input_name FROM uralensis_record_track_template
          WHERE temp_status='1' AND temp_assignee_id=" . $user_id);
        }
        return $query->result();
    }
 ...
 public function get_track_record_templates_by_id($user_id)
    ...
    public function get_doctors()
    {
        ...
        $result = array();
        if ($is_admin || $groupType = 'L' || $groupType = 'H' ) 
 		{
            $sql = "
        ...
        WHERE `users`.user_type = 'D'";
            ...
            $user_list = $this->db->query("SELECT user_id FROM `users_groups` where user_id in (select user_id from users_groups inner join `groups` on `groups`.`id` = users_groups.group_id where group_type = 'D') and institute_id = $group_id")->result_array();
            $list = [];
          ...
          AES_DECRYPT(email, '" . DATA_KEY . "') AS email, 
          users.id,AES_DECRYPT(username, '" . DATA_KEY . "') AS username,
		  profile_picture_path as profile_pic 
          FROM users
    ...
    public function get_clinic()
    {
        ...
        $sql = "SELECT AES_DECRYPT(phone, '" . DATA_KEY . "') AS phone,AES_DECRYPT(company, '" . DATA_KEY . "') AS company,AES_DECRYPT(last_name, '" . DATA_KEY . "') AS last_name,AES_DECRYPT(first_name, '" . DATA_KEY . "') AS first_name, AES_DECRYPT(email, '" . DATA_KEY . "') AS email, users.id,AES_DECRYPT(username, '" . DATA_KEY . "') AS username FROM users WHERE `users`.user_type = 'C'";
        $query = $this->db->query($sql);
        return $query->result();
    }
...
//        $collection_time = date('H:i:s', strtotime($this->input->post('collection_time')));
    ...
    public function get_user_couriers($courier_id = FALSE,$sender_type=FALSE)
    {
        ...
        $user_id = $this->ion_auth->user()->row()->id;
        $groupRowData = $this->ion_auth->get_users_groups()->row();
        ...
        $this->db->select("tbl_courier.*,DATE_FORMAT(tbl_courier.created_at,'%d-%m-%Y') as created_at,
                           ...
                           courier_companies.name as courier_company,courier_companies.logo as company_logo, 
