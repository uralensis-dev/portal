...
Class Doctor_model extends CI_Model {
	 ...
	 * @return array
	 */
	public function display_published_record($year, $recent = '', $flag_type = '', $sort_authorize = '', $urgency_type = '', $row_color_code = '', $viewType = '') {
	...
	public function doctor_record_list($doctor_id, $filter = '') {
		...
		$query = $this->db->query("SELECT *,request.request_datetime as stDate,tbl_courier.collection_date as collection_date_custom, CASE WHEN request_assignee.user_id = $doctor_id THEN 'assigned' Else 'unassigned' END AS user_type FROM request
...
//        echo $this->db->last_query(); exit;
	...
	public function doctor_record_detail($id) {
		...
		*/
		...
		tbl_courier.collection_date as collection_date_custom, request.request_datetime as stDate 
        FROM request
        INNER JOIN request_assignee ON request_assignee.request_id = request.uralensis_request_id
        LEFT JOIN request_autopsy_detail apd ON apd.request_id = request.uralensis_request_id
        LEFT JOIN request_virology_detail vrd ON vrd.request_id = request.uralensis_request_id
	...
	public function doctor_record_detail_specimen($id, $hospital_group_id = '') {
		...
		$sql = "SELECT users.id,AES_DECRYPT(phone, '" . DATA_KEY . "') AS phone,
                ...
                rbc.billing_type, rbc.bill_code, rbc.bill_code_text, rbc.billing_type, rbc.bill_description, rbc.bill_price,urtt.billing_type as billing_type2, urtt.specimen_id_val as specimen_id_val, urtt.tissue_type_id, urtt.department_id, tbl_courier.collection_date as collection_date_custom
                 FROM request
            INNER JOIN specimen
            INNER JOIN users
            INNER JOIN request_assignee
	...
	*/
	...
	public function get_clinician_and_derm(
		...
		$user_data = $this->db->select('id, username, first_name, last_name')
			...
			->get('users')
			->result_array();
		if ($copy_to != '' && $copy_to == 'copy_to') {
			$user_data = $this->db->select('id, username, first_name, last_name')
				->where('user_type', 'C')
				->where('id !=', $db_value)
				->get('users')
				->result_array();
			$copy_to_clinician = $this->db->select('GROUP_CONCAT(clinician_id) as clinicianIds')
				->where('request_id', $request_id)
				->get('request_clinicians')
				->row_array();
			if (count($copy_to_clinician) > 0) {
				$clinicianIds = explode(",", $copy_to_clinician['clinicianIds']);
			}
		}
		...
		if (!empty($user_result)) {
			foreach ($user_result as $key => $value) {
				...
				if ($copy_to != '' && $copy_to == 'copy_to') {
					if (count($clinicianIds) > 0) {
						if (in_array($value['id'], $clinicianIds)) {
							$select = 'selected';
						}
					}
				}
