...
//        $hospital_id = $this->ion_auth->get_users_groups()->row()->id;
	...
	{	        
        ...
        foreach ($response as $k=>$row){
            ...
            $response[$k]['employee_count'] = 0;
            if($row['hospital_id'] != ''){
                $subEmpQuery = "select COUNT(id) as emp_count from users_groups where users_groups.institute_id=".$row['hospital_id'];
    ...
    public function get_lab_users_count($hospital_id = '', $group_id = '') {
        $lab_id = $this->ion_auth->get_users_main_groups()->row()->id;
        if($group_id != ''){
            $lab_id = $group_id;
        }
...
public function status_bar_result_count_published()
    {
       ...
       // $result = $this->db->query("SELECT * FROM request where specimen_publish_status = 1 and lab_id IN ($labIds)");
        ...
        $result = $this->db->query("SELECT *, CONCAT(AES_DECRYPT(users.first_name, '" . DATA_KEY . "'),' ' ,AES_DECRYPT(users.last_name, '" . DATA_KEY . "')) AS added_by, tbl_courier.courier_no as courier_number, count(DISTINCT(specimen.specimen_id)) as speciman_no FROM request
            INNER JOIN request_assignee                     
...
//        $group_id = $this->ion_auth->get_users_groups($user_id)->row()->id;
    ...
    public function get_lab_templates($user_id='', $clinic_id = '') {
        ...
        if(!empty($clinic_id)){
            $this->db->where('li.hosp_id', $clinic_id);
        }
        return $this->db->get()->result();
    }
    // public function get_clinic($id){
    //     $this->db->select('gr.description as clinic_name, gr.first_initial, gr.last_initial, hi.hosp_address, hi.hosp_country, hi.hosp_city, hi.hosp_state, hi.hosp_post_code, hi.hosp_email, hi.site_identifier, hi.identifier, hi.hosp_phone, hi.hosp_mobile, hi.hosp_fax, hi.hosp_website, hi.group_id, hi.logo, hi.hosp_id, hi.billing_type');
    ...
    // }
	public function get_clinic($id){
        $this->db->select('gr.description as clinic_name, gr.first_initial, gr.last_initial, hi.hosp_address, hi.hosp_country, hi.hosp_city, hi.hosp_state, hi.hosp_post_code, hi.hosp_email, hi.site_identifier, hi.identifier, hi.hosp_phone, hi.hosp_mobile, hi.hosp_fax, hi.hosp_website, hi.group_id, hi.logo, hi.hosp_id, hi.billing_type, p.hospital_id');
        $this->db->from('hospital_information hi');        
        $this->db->join('groups gr', 'gr.id = hi.group_id');
        $this->db->join('patients p', 'p.hospital_id=hi.group_id', 'left');
        $this->db->where('gr.group_type', 'H');
        $this->db->where('hi.hosp_id', $id);
        $query = $this->db->get();        
        return $query->row_array();
