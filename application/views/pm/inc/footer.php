...
<!-- Datetimepicker JS -->
<script src="<?php echo base_url('/assets/newtheme/plugins/summernote/dist/summernote.min.js'); ?>"></script>
<!-- Datatable JS -->
    ...
    $(document).ready(function () {
     ...
     --------------------------------------*/
        ...
        $('.action-select').on('click', function (e) {
            ...
            if (dataVal == "all") {
                $(".user-email-card").show();
            } else {
                $(".user-email-card").hide();
                $(`[data-status="${dataVal}"]`).show();
            }
        });
...
<script>
    ...
    function deletedraft(id) {
        $.ajax({
            url: '<?php echo base_url('/index.php/pm/deletedraft'); ?>',
            type: 'POST',
            global: false,
            dataType: 'json',
            data: {"id": id},
    ...
    function createlabel() {
        $.ajax({
            url: "<?php echo base_url('/index.php/pm/createlabel'); ?>",
            method: "POST",
            dataType: "json",
            data: {"groupname": $("#group_name").val(), "user_id": "<?php echo $this->ion_auth->user()->row()->id?>"},
            dataType: "json",
            success: function (data) {
                alert(data.msg);
                window.setTimeout(function () {
    ...
    function markasread(id) {
        ...
        $.ajax({
            url: '<?php echo base_url('/index.php/pm/markasread'); ?>',
            type: 'POST',
            global: false,
            dataType: 'json',
            data: {'messages_id': $("#messages_id").val(), "id": id},
            success: function (data) {
                if (data.type === 'success') {
                    alert(data.msg);
                    window.location.reload(1);
    ...
    function markaslabeled(id) {


        $.ajax({
            url: '<?php echo base_url('/index.php/pm/markaslabeled'); ?>',
            type: 'POST',
            global: false,
            dataType: 'json',
            data: {'messages_id': $("#messages_id").val(), "id": id},
    ...
    function clearMessages(id) {
        ...
        $.ajax({
            ...
            global: false,
            dataType: 'json',
            data: {'messages_id': $("#unreadmessages").val(), "id": 1},
            success: function (data) {
                if (data.type === 'success') {
                    alert(data.msg);
                    window.location.reload(1);
