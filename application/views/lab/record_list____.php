...
<div class="container-fluid">
			...
			<div class="row">
				<div class="col-sm-12">
					<ul class="list-track">
						...
						<li>
							<a href="#" class="filter_by_status_btn" data-toggle="collapse" title="Lab Entry"><svg x="0px" y="0px" width="45.5px" height="40.9px" viewBox="0 0 45.5 40.9">
			...
			?>
				<?php if ($sr_specialty != '') {
					...
					<tr class="<?php //echo $row_code; 
						...
						<td class="<?php // echo $row_code; 
									?>">
							<a title="Edit" href="<?php echo site_url() . '/doctor/doctor_record_detail_old/' . $row->uralensis_request_id . $request_type; ?>"><?php echo $row->lab_number; ?></a><br><?php echo $row->ura_barcode_no; ?>
						</td>
						<td>
							<?php
							$f_initial = '';
						...
						<td><?php echo uralensisGetUsername($row->dermatological_surgeon, 'fullname'); ?></td>
						...
						<!--<td><br><?php //echo $lab_release_date; 
									?></td>-->
						...
						<td class="flag_column text-center">
							<span class="icon-try">
								...
								<div><img <?php if ($has_slide) echo 'onclick = "window.location.href = \'' . base_url() . 'doctor/doctor_record_detail/' . $row->uralensis_request_id . '/view\'"' ?> src="<?php if ($has_slide) echo base_url('/assets/icons/vslideico_green.png');
									...
									<div class="modal-content">
										...
										<div class="modal-body">
											...
											<form method="post" class="form flag_comments" id="flag_comments_form_<?= $flag_count; ?>">
												<div class="form-group">
													<textarea name="flag_comment" id="flag_comment" rows="5" class="form-control flag_comment" disabled></textarea>
						...
						<td style="text-align:right; padding-right: 10px;">
							...
							<a title="Delete"  href="<?php echo site_url() . '/lab/delete_record/' . $row->uralensis_request_id . $request_type; ?>" onclick="javascript:confirm('Do you really wants to delete this records')"><i class="fa fa-trash-o m-r-5"></i> </a>
 ...
 <div class="dropdown dropdown-action">-->
							...
							<!--                                            <a class="dropdown-item" href="--><?php ////echo site_url() . '/doctor/doctor_record_detail/' . $row->uralensis_request_id; 
																												?>
							<!--"><i class="fa fa-pencil m-r-5"></i> Edit</a>-->
							<!--                                            --><?php //$request_type = ($row->speciality_group_id=='2'?'/postmortem':''); 
																				?>
							<!--                                            <a class="dropdown-item" href="--><?php //echo site_url() . '/doctor/doctor_record_detail_old/' . $row->uralensis_request_id.$request_type; 
																												?>
							<!--"><i class="fa fa-pencil m-r-5"></i> Edit</a>-->
							...
							<!--                                        </div>-->
							<!--                                    </div> -->
							<!-- <a href="<?php //echo site_url() . '/doctor/doctor_record_detail/' . $row->uralensis_request_id; 
											?>" class="btn btn-info btn-sm"><i class="lnr lnr-pencil"></i></a> -->
						</td>
						<td class="hide_content">
							<p style="display:none;"><?php echo $row->flag_status; ?></p>
						</td>
						<td class="hide_content">
							<p style="display:none;"><?php // echo $row_code; 
														?></p>
						</td>
						<td class="hide_content">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$(document).on("click", ".delete_record", function() {
			var url = $(this).data('custom_url');
			// alert(url)
			// Get User Login Token Here
			jQuery.ajax({
				type: "GET",
				...
				success: function(data) {
					console.log(data);
					let passcode = prompt("Please Enter Passcode To Delete User Record");
					if (passcode == data) {
						
						jQuery.sticky('Record Deleted Successfully', {
							classList: 'success',
							speed: 200,
							autoclose: 5000
						});
