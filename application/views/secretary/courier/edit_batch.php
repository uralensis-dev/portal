...
<div class="row">
    <div class="col-md-12">
        ...
        ?>
        <form class="form" id="edit_batch_form" method="post" action="<?php echo base_url('index.php/secretary/process_edit_batch'); ?>">
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="batch_ref">Batch Reference</label>
                        <input class="form-control" name="batch_ref" readonly value="<?php echo!empty($batch_ref) ? html_purify($batch_ref) : ''; ?>" id="batch_ref">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" name="courier_cost_code" value="<?php echo!empty($courier_cost_code) ? html_purify($courier_cost_code) : ''; ?>" id="courier_cost_code">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="batch_courier" id="batch_courier" class="form-control">
                            ...
                            if (!empty($courier_list)) {
                                foreach ($courier_list as $courier) {
                                    ...
                                    echo '<option ' . html_purify($selected) . ' value="' . intval($courier->ura_courier_id) . '">' . html_purify($courier->ura_courier_name) . '</option>';
                                }
                            }
                            ?>
                        </select>
                    ...
                    <div class="form-group">
                        <label for="courier_track_no">Courier Track Number</label>
                        <input class="form-control" name="courier_track_no" value="<?php echo !empty($courier_track_no) ? html_purify($courier_track_no) : ''; ?>" id="courier_track_no">
                    </div>
                </div>
            </div>
            <hr>
            ...
            <input type="hidden" name="batch_id" value="<?php echo intval($batch_id); ?>">
            ...
            <table class="table table-condensed" id="clinic_batches_list">
                ...
                <tbody>
                    <?php foreach ($clinics_list as $clinics) { ?>
                        <tr>
                            <td><?php echo html_purify($clinics->ura_clinic_ref_no); ?></td>
                            <td><?php echo html_purify($clinics->ura_clinic_loca); ?></td>
                            <td><?php echo html_purify($clinics->ura_clinic_lead); ?></td>
                            <td><?php echo html_purify($clinics->ura_clinic_total_patients); ?></td>
                            <td><?php echo html_purify($clinics->ura_clinic_total_samples); ?></td>
                            <td><?php echo html_purify($clinics->ura_clinic_imf_samples); ?></td>
                        </tr>
                    <?php } ?>
                </tbody>
            </table>
