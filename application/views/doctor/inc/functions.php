...
function get_uralensis_username($user_id) {
    ...
    if (!empty($user_id)) {
       ...
       /* $f_name = $ci->ion_auth->user($user_id)->row()->first_name;
        ...
        $username = $getdetails->first_name . ' ' . $getdetails->last_name;
 ...
 * @return void
 */
function uralensis_get_patient_data($record_id) {
    $ci = & get_instance();
    if(!empty($record_id) && !empty($type)) 
	{		
		$query = $ci->db->query("SELECT * FROM request INNER JOIN `patients`
        WHERE request.patients_id = patients.id
        AND request.uralensis_request_id = $record_id"
        );
        if ($ci->$query->num_rows() > 0) {
            foreach ($ci->$query->result_array() as $row) {
                $data[] = $row;
            }
        }
        return $query->result();
...
function get_patient_Metadata($record_id) {
    $ci = & get_instance();
    if(!empty($record_id)) 
	{		
		$query = $ci->db->query("SELECT * FROM request INNER JOIN `patients`
 ...
 * @return void
 */
function get_next_previous_records($unpublish_list = array(), $record_id = '', $wrap = false, $get_type = '') {
        ...
        if (array_key_exists($record_id, $unpublish_list)) {
            ...
            if (!empty($get_type) && $get_type === 'prev') {
                ...
                <div class="tg-previousrecord">
                    <a href="<?php echo base_url('index.php/doctor/doctor_record_detail/' . $return['prev']); ?>">
                ...
                <div class="tg-nextecord">
                    <a href="<?php echo base_url('index.php/doctor/doctor_record_detail/' . $return['next']); ?>">
