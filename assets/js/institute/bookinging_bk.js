...
$("#template_id").val(data.data['templateid']);
                ...
                $("#dateandtime_pre").val(data.data['timestamp']);
                if ((data.data['timestamp']).length > 0) {
                    $("#dateandtime_pre").addClass('blue-border');
                }
...
function getsearchbarcode() {
    ...
    $('.barcode_no_search').on('click', function () {
        ...
        var val = $("#template_name").val();
        is_template_select = val.length > 0;
        
        if (!is_template_select) 
		{
            _this.parents('.tg-inputicon').append('<i class="fa fa-spin fa-spinner"></i>');
            $.ajax({
                url: `${_base_url}institute/search_and_add_barcode_record`,
                type: 'POST',
                global: false,
                dataType: 'json',
                ...
                success: function (data) {
                    if (data.type === 'success') {
                        setTimeout(function () {
                            jQuery.sticky(data.msg, {classList: 'success', speed: 200, autoclose: 7000});
                            $('.tracking_search_result').html(data.encode_data);
                            show_flags_on_hover();
                            change_flag_status();
                            flag_tooltip();
        ...
        $.ajax(
            {
                ...
                method: 'POST',
                data: {patient_id: $("#booking_patient_id").val(), speciality_id: $('#bookingin_sepciality_id').val(), lab_id: $('#bookingin_lab_id').val(),tem_id:$('#template_name').val(), status_code: 'Booked In To Lab'},
                success: function(data) {                    
                    if (data.type === 'success') {
                        setTimeout(function () {
                            jQuery.sticky(data.msg, {classList: 'success', speed: 200, autoclose: 7000});
