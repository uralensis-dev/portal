...
$(() => {
    ...
    $.get(baseUrl + 'auth/get_all_roles1', function (data) {
        ...
        console.log(data,"Sdfdsf");

        // select.append(all_option);
        for (role of data['H']) {
            
            HosGroup += `<option data-groupType="H" value="${role.group_type}" data-group="Hospital">${role.description}</option>`;
        }
        for (role of data['L']) {
            LabGroup += `<option data-groupType="L" value="${role.group_type}" data-group="Laboratory">${role.description}</option>`;
        }
        for (role of data['D']) {
            PathGroup += `<option data-groupType="D" value="${role.group_type}" data-group="Pathologist">${role.description}</option>`;
        }
		for (role of data['S']) {
            SuGroup += `<option data-groupType="S" value="${role.group_type}" data-group="Surgeon">${role.description}</option>`;
        }
        for (role of data['A']) {
            SaGroup += `<option  data-groupType="A" value="${role.group_type}" data-group="System Admin">${role.description}</option>`;
        }
    ...
    $('#user_role_lab').change(function (){
        ...
        var group = $('option:selected', this).attr('data-groupType');
        if(typeof role === 'undefined'){
            role = "Clinic";
            group = "H";
        }
        //Getting role based group
        var csrf_token_name = $("#csrf_token_name").val();
        var csrf_token_hash = $("#csrf_token_hash").val();
        $.ajax({
            type: "POST",
            url: baseUrl + '/auth/getRoleBasedGroup',
            data: {[csrf_token_name]: csrf_token_hash,group : group,role : role},
            success: function (response) {
                $('#clinic_id').html(response);
            }
        });
        $('#roleWrap').html(role);
    });
