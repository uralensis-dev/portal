...
function barcode_type(data, atype){
    ...
    $('#spePathologist').val(data['pathlogositName']);
    $('#specimCount').val(data['specimentCounts']);
    $('#specimBlockId').val(data['spBlockId']);
    $('#tetsIds').val(data['test']);
    $('#btn_download_request_cassete, #btn_download_request_slide').attr("data-values", data['lab_no']+"-"+data['blockNo']+" "+data['patientName']+" " + data['pathlogositName']);
...
function barcode_p(row, action_type) {
    ...
    if(action_type == 1){
        $('.requestData, .testData, .patientData').removeClass('hide');
    }else if(action_type == 2){
        $('.barcodeData').removeClass('hide');
    }else{
        $('.requestData').removeClass('hide');
    }
    // var digi_number = $('#digi_number').html();
    var digi_number = $('#lab_number').val();
...
function generate_barcode(code, br_data, action_type = 1){
    if(code != ''){
        ...
        $.ajax({
            url: '../../../GenerateBarcode',
            type: "post",
            data: {[csrf_name]: csrf_hash, 'code' : code, save_it: 1, 'br_data': br_data, 'patient_id' : patient_id, 'action_type' : action_type, pathologist : $('#spePathologist').val(), patientName : $('#patientName').val(), tetsIds : $('#tetsIds').val(), specimCount : $('#specimCount').val(), tests : testString},
            success: function (response) {
